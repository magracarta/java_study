...
public class Application {
    public static void main(String[] args) {
        ...
        * 클래스 또한 부모클래스와 자식 클래스로 나누어서 부모가 가지는 멤버를 자식에 물려받아
        * 자기의 멤버인 거처럼 사용할 수 잇도록 만든 기술이다.
        *
        ...
        * 정의한 메소드대로 동작하도록 구현 물체 부분을 새롭게 다시 작성하는 기술이다.
        * 메소드 재정의를 하면 메소드를 호출할 시 재정의한 메소드가 우선적으로 동작하게 된다.
        * */


        /*
        * 이러한 상속이라는 기술을 사용하게 되면 얻게되는 두 가지가 있다.
        ...
        *  */
        ...
        /*
        * 상속으로 인한 단점
        * 1. 부모 클래스의 기능을 추가, 변경시에 자식 클래스가 정상 작동하는지 예측이 힘들다.
        *   상속 구조가 복잡해질 수록 그 영향에 대한 예측이 힘들어 이런 단점이 유지보수성이 증가한다는 장점과 반대로
        *   유지보수에 악영향을 미친다.
        * 2. 또한 부모클래스 변경 또한 쉽지 않다. 자식 클래스에서 중요하게 사용하는 기능인 경우 부모클래스 변경할 시
        *   자식 클래스에 모두 영향을 줄 수 있다. 역시 유지모수에 악영향을 미친다.
        * 3. 부모클래스에서는 의미 있었던 기능들이 자식 클래스에서는 무의미 할 수 있다.(불필요한 기능이 추가됨)
        * */
        ...
        * 그래야 자식클래스로 생성한 객체들이 서로 역할을 수행해가며 유연한 코드를 작성 할 수 있게된다.
        * 동일한 역할을 가지는 모든 객체는 동일한 메세지를 수신하기는 하지만 객체별로 그 메시지에 등답하는 방식이
        * 서로 다를수 있다.
        * */
